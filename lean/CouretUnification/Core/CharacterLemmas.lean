...
lemma sum_left_mul_eq_sum (f : G → ℂ) (a : G) :
  ...
  classical
  simpa using (Fintype.sum_equiv (Equiv.mulLeft a) f)
...
lemma sum_char_eq_zero_of_ne_one (χ : Char G) (hχ : χ ≠ 1) :
  ...
  · obtain ⟨a, ha⟩ := not_forall.mp htriv
    ...
    have hshift : χ a * S = S := by
      calc χ a * S = ∑ g : G, χ a * χ g := by simp [S, Finset.mul_sum]
        _ = ∑ g : G, χ (a * g) := by
            refine Finset.sum_congr rfl ?_; intro g _; simpa [char_mul]
        _ = ∑ g : G, χ g := by
            simpa [S] using sum_left_mul_eq_sum (f := fun g => χ g) a
